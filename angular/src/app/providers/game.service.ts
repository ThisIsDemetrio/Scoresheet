...
export class GameService {
	...
	getGamesByGroupId(groupId: string): Observable<Game[]> {
		return this.httpClient.get<Game[]>(`${this.endpoint}GetByGroup/${groupId}`);
	}
	...
	updateGame(game: Game): Observable<boolean> {
		return this.httpClient.put<boolean>(`${this.endpoint}Update/${game.id}`, {
			game,
		});
	}

	deleteGame(gameId: string): Observable<boolean> {
		return this.httpClient.delete<boolean>(`${this.endpoint}Delete/${gameId}`);
	}
}
